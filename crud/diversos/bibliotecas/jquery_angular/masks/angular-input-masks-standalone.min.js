...
(function() {
  "use strict";
  var app = angular.module("sitesudc");
  app.directive("prnumberspinner", directivectrl);


  ///////////////////////////////////////////////////////////////
  // define
  directivectrl.$inject = ['$timeout', '$compile'];

  function directivectrl($timeout, $compile) {

    ///<summary>
    //    /// Usage:
    //<div prnumberspinner isrequired="false" incrementvalue="0.01" decimals="2"
    //    item-value="svt.servicetypemodel.BudgetedHours" ng-model="svt.servicetypemodel"
    //    max="1000" min="0" fieldname="[[[Targeted Hours]]]"
    //    inputsize="small"></div>
    ///
    ///</summary>
    ///<returns></returns>
    ...
    var directive = {
      ...
      template: '<div>' +
        '<div class="input-group spinner-sm">' +
        ...
        ' max="{{max}}"' +
        ' id={{inputid}}' +
        ' ng-attr-name="{{fieldname}}">' +
        '<div class="input-group-btn-vertical">' +
        '<button class="btn btn-default" type="button" ng-disabled="updisable" ng-click="upClicked($event)"><i class="fa fa-caret-up"></i></button>' +
        '<button class="btn btn-default" type="button" ng-disabled="downdisable" ng-click="downClicked($event)"><i class="fa fa-caret-down"></i></button>' +
        '</div> </div>' +
        '<div ng-show="notvalid" class="has-error">[[[Please enter a valid number between {{minvalue}} and {{maxvalue}} ]]] </div>' +
        '</div>',
      link: linker
    };
    ...
    function linker(scope, element, attrs, modelCtrl) {

      scope.inputid = scope.$id + "input";
      scope.notvalid = false;
      scope.updisable = false;
      scope.downdisable = false;


      var inputelement = [];
      if (scope.inputsize == "medium") {
        inputelement = element.find(".spinner-sm");
        inputelement.removeAttr("class");
        inputelement.attr('class', 'input-group spinner-md');
      } else if (scope.inputsize == "large") {
        inputelement = element.find(".spinner-sm");
        inputelement.removeAttr("class");
        inputelement.attr('class', 'input-group spinner-lg');
      } else {
        inputelement = element.find(".spinner-sm");
        inputelement.removeAttr("class");
        inputelement.attr('class', 'input-group spinner-sm');
      }


      scope.upClicked = function($event) {
        // get the input value
        var inputValue = parseInt(scope.itemValue);
        var totalvalue = 0;
        if (angular.isNumber(inputValue)) {
          // Now check the dev as specified float
          if (angular.isNumber(scope.decimals) && scope.decimals > 0) {
            // parse the input as decimal
            inputValue = parseFloat(scope.itemValue);
            totalvalue = (parseFloat(scope.incrementvalue) + parseFloat(inputValue)).toFixed(1);
          } else {
            // parse the input as decimal
            inputValue = parseInt(scope.itemValue);
            totalvalue = parseInt(scope.incrementvalue) + inputValue;
          }

          if (totalvalue <= scope.max) {
            scope.itemValue = totalvalue;
            //  scope.downdisable = false;
          } else {
           // scope.updisable = true;
           // scope.downdisable = false;
          }
        }
      ...
      scope.downClicked = function($event) {
        // get the input value
        var inputValue = parseInt(scope.itemValue);
        var totalvalue = 0;
        if (angular.isNumber(inputValue)) {
          ...
          if (angular.isNumber(scope.decimals) && scope.decimals > 0) {
            // parse the input as decimal
            inputValue = parseFloat(scope.itemValue);
            totalvalue = inputValue - parseFloat(scope.incrementvalue);
          } else {
            // parse the input as decimal
            inputValue = parseInt(scope.itemValue);
            totalvalue = inputValue - parseInt(scope.incrementvalue);
          }
          ...
          if (totalvalue >= scope.min) {

            scope.itemValue = totalvalue;
          } else {
          //  scope.updisable = false;
           // scope.downdisable = true;
          }
        }
      ...
      // set the form dirty means something has been changed
      var formdirty = function() {
        if (scope.notformfield) {
          modelCtrl.$setDirty = angular.noop
        } else {
          modelCtrl.$setDirty();
        }
